...
import { Upload, File, X, CheckCircle2, AlertCircle, Loader2, ArrowLeft, Zap, Heart } from 'lucide-react';
    ...
    <div className="min-h-screen bg-zinc-950 text-white p-6 pb-24 font-sans">
      <header className="mb-8 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-zinc-500 text-xs">Session: {sessionId.slice(0, 8)}...</p>
          </div>
        </div>
        <div className="text-zinc-500 hover:text-white transition-colors cursor-pointer" onClick={() => router.push('/')}>
          <X className="w-6 h-6" />
        </div>
      </header>
      ...
      <main className="max-w-md mx-auto">
        ...
        >
          ...
          <p className="font-medium">Tap to select files</p>
          <p className="text-zinc-500 text-sm mt-1">or drag and drop them here</p>
        </div>

        {/* Upload List */}
        <div className="mt-10 space-y-4">
          <h2 className="text-sm font-bold uppercase tracking-widest text-zinc-500 mb-4 px-2">
            Upload Status
          </h2>
          <AnimatePresence initial={false}>
            {uploadingFiles.length === 0 ? (
              <p className="text-center text-zinc-600 py-8 italic">No files selected yet</p>
            ) : (
        ...
        {uploadingFiles.some(f => f.status === 'completed') && (
          ...
          >
            <button 
              ...
              className="w-full bg-white text-black font-bold py-4 rounded-2xl shadow-xl hover:bg-zinc-200 transition-colors"
            >
              Clear Uploads
            </button>
          </motion.div>
